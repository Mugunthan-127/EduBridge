...
class KolibriCoreConfig(AppConfig):
    ...
    def ready(self):
        ...
        # EduBridge SMS Gateway Extension
        try:
            from django.db.models.signals import post_save
            from kolibri.core.logger.models import MasteryLog
            def send_sms_on_mastery(sender, instance, created, **kwargs):
                if getattr(instance, 'complete', False):
                    logger.info("EduBridge SMS Gateway: Sent 'Mastery Achieved' SMS to parent of user %s for log %s", instance.user.username, instance.id)
            post_save.connect(send_sms_on_mastery, sender=MasteryLog)
        except Exception as e:
            logger.error("Failed to initialize EduBridge SMS Gateway: %s", e)
